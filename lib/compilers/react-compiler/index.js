...
console.log('bundlePath', bundleFilePath);
...
${assignmentStatements}
${fnDeclarations}
console.log('usercomp 2', UserComp);
var CompUser = React.createClass({
    render() {
  ...
  // pass a number to set the polling interval
    ...
    if (err) {
      ...
      try {
        ...
        return subscriber.next(React.createElement('div', {style: {background: 'red', color: 'black'}}, e.toString()))
      } finally {
        global.eval = previousEval
      }

      let App = React.createElement(bundle.default)

      if (App) {
        return subscriber.next(App)
      } else {
        return subscriber.next(new Error('error while evaluating bundle'))
      }
    }
  })
}
...
function onCodeChange (code, playgroundCode, openFilePath) {
  // console.log('code change', code, playgroundCode, openFilePath)
